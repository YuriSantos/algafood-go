...
func (s *CozinhaService) Save(cozinha *model.Cozinha) error {
	...
	if s.cacheSvc != nil {
		if err := s.cacheSvc.InvalidateCozinha(cozinha.ID); err != nil {
			log.Printf("Aviso: Falha ao invalidar cache da cozinha %d: %v", cozinha.ID, err)
		}
	}
...
func (s *CozinhaService) Delete(id uint64) error {
	...
	if s.cacheSvc != nil {
		if err := s.cacheSvc.InvalidateCozinha(id); err != nil {
			log.Printf("Aviso: Falha ao invalidar cache da cozinha %d: %v", id, err)
		}
	}
