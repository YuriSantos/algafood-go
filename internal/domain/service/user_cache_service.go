...
// InvalidateAllUsers remove todos os usuários do cache
func (s *UserCacheService) InvalidateAllUsers() error {
	ctx, cancel := context.WithTimeout(context.Background(), cacheTimeout*10) // Mais tempo para operação em lote
	defer cancel()

	// Busca todas as chaves de usuário
...
// GetAuthorities obtém as authorities de um usuário do cache (operação rápida)
...
// WarmUpCache pré-carrega usuários no cache (opcional, para startup)
func (s *UserCacheService) WarmUpCache(users []*model.Usuario) {
	...
	log.Printf("Cache aquecido com %d usuários", len(users))
}
